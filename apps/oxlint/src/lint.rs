...
const DEFAULT_CONFIG_NOTE_MESSAGE: &str = "\u{1b}[33mUsing Oxlint's default configuration. Run `oxlint --init` to set up Oxlint in this project.\u{1b}[0m\n";
...
impl CliRunner {
    ...
    fn run_with_stderr(self, stdout: &mut dyn Write, stderr: &mut dyn Write) -> CliRunResult {
        ...
        {
            print_and_flush_stdout(stderr, DEFAULT_CONFIG_NOTE_MESSAGE);
        }
...
mod test {
    ...
    fn default_config_note_when_no_config_found() {
        ...
        assert!(stderr.contains(DEFAULT_CONFIG_NOTE_MESSAGE.trim_end()));
    }
